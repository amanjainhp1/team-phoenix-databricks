...
ltf_name AS
(
    SELECT DISTINCT forecast_name, version
...
*/
(
    SELECT CONCAT('LTF',' - ','20',SUBSTRING(ltf.forecast_name,5,2),'-', FORMAT(cal.month,'00')) AS report_name
        ,a.platform_subset
        ,d.pl
    ...
    LEFT JOIN mdm.product_line_xref e
        ON d.pl=e.pl
    LEFT JOIN product_line_scenarios_xref plx
        ON d.pl=plx.pl
...
(
    ...
    WHERE 1=1
        ...
        AND forecast_name = CONCAT('FLASH',' - ',YEAR(GETDATE()),'-','04')
        AND c.day_of_month = 1
),
...
(
    SELECT DISTINCT
        ...
        LEFT JOIN prod.flash_wd3 b
            ON 1=1
            AND a.record = 'FLASH'
            AND a.forecast_name=b.forecast_name
...
*/
(
SELECT a.forecast_name AS report_name
    ,b.platform_subset
    ,f.region_3
    ,f.region_5
    ,f.market10
...
INNER JOIN product_line_xref AS d
    ON c.pl = d.pl
...
(
SELECT
    CONCAT(a.record,' ',e.fiscal_yr) AS report_name
    ,b.platform_subset
    ,c.pl
    ,d.l6_description AS pl_name
    ,c.hw_product_family
