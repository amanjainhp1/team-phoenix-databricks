...
FROM (
        SELECT 'BASE_PRODUCT'::character varying AS record_type,
            ...
            CASE
                WHEN forecast_supplies_baseprod.base_product_line_code::text = '5T'::character varying::text
                OR forecast_supplies_baseprod.base_product_line_code::text = 'GJ'::character varying::text
                OR forecast_supplies_baseprod.base_product_line_code::text = 'GK'::character varying::text
                OR forecast_supplies_baseprod.base_product_line_code::text = 'GP'::character varying::text
                OR forecast_supplies_baseprod.base_product_line_code::text = 'IU'::character varying::text
                OR forecast_supplies_baseprod.base_product_line_code::text = 'LS'::character varying::text THEN 1
                ELSE 0
            END AS is_canon,
            ...
            COALESCE(
                forecast_supplies_baseprod.baseprod_contra_per_unit,
                0::double precision
            ) * COALESCE(
                forecast_supplies_baseprod.insights_base_units,
                0::double precision
            ...
            COALESCE(
                forecast_supplies_baseprod.baseprod_revenue_currency_hedge_unit,
                0::double precision
            ) * COALESCE(
                forecast_supplies_baseprod.insights_base_units,
                0::double precision
            ...
            COALESCE(
                forecast_supplies_baseprod.baseprod_variable_cost_per_unit,
                0::double precision
            ) * COALESCE(
                forecast_supplies_baseprod.insights_base_units,
                0::double precision
            ...
            COALESCE(
                forecast_supplies_baseprod.baseprod_fixed_cost_per_unit,
                0::double precision
            ) * COALESCE(
                forecast_supplies_baseprod.insights_base_units,
                0::double precision
        ...
        FROM fin_prod.forecast_supplies_baseprod forecast_supplies_baseprod
            JOIN mdm.iso_country_code_xref iso_country_code_xref ON forecast_supplies_baseprod.country_alpha2::text = iso_country_code_xref.country_alpha2::text
            AND forecast_supplies_baseprod.version::text = (
                (
            ...
            JOIN mdm.supplies_xref supplies_xref ON supplies_xref.base_product_number::text = forecast_supplies_baseprod.base_product_number::text
            JOIN fin_prod.list_price_version lpv ON lpv.version::text = forecast_supplies_baseprod.sales_gru_version::text
    ) base_pl
    JOIN (
        SELECT sup.cal_date,
            ...
            CASE
                WHEN (
                    SUM(sup.adjusted_cartridges) OVER (
                        PARTITION BY sup.cal_date,
                        sup.geography,
                        sup.base_product_number ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
                    )
                ) = 0::double precision THEN NULL::double precision
                ELSE sup.adjusted_cartridges / (
                    SUM(sup.adjusted_cartridges) OVER (
                        PARTITION BY sup.cal_date,
                        sup.geography,
                        sup.base_product_number ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
        ...
        WHERE sup.version::text = (
                (
                    SELECT max(working_forecast.version::text) AS max
                    FROM prod.working_forecast
                )
            )
            AND sup.cal_date >= (
                (
                    SELECT MIN(forecast_supplies_baseprod.cal_date) AS min
                    FROM fin_prod.forecast_supplies_baseprod
                )
            )
            AND sup.adjusted_cartridges <> 0::double precision
            AND sup.geography_grain::text = 'MARKET10'::character varying::text
        GROUP BY sup.cal_date,
            sup.geography,
            sup.platform_subset,
            sup.base_product_number,
            sup.adjusted_cartridges
    ) mix ON base_pl.cal_date = mix.cal_date
    AND base_pl.market10::text = mix.market10::text
    AND base_pl.product_number::text = mix.base_product_number::text
    JOIN mdm.hardware_xref hw ON mix.platform_subset::text = hw.platform_subset::text;

ALTER TABLE financials.v_finance_forecast_pl OWNER TO auto_glue;
GRANT ALL ON financials.v_finance_forecast_pl TO group phoenix_dev;