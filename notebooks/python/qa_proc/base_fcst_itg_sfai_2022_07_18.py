...
# MAGIC %md
# MAGIC ## Laser Compare - row to row compare
# MAGIC 
...
from IE2_Staging.dbt.c2c_cartridges_w_vtc_20220715 as vtc
...
fig = px.line(data_frame=viz_df_2,
              ...
              line_group='variable',
              color='variable',
              title='RS - SFAI Base Fcst compare - LASER')
...
# MAGIC %md 
# MAGIC ## Ink Fcst Compare - time series
...
where 1=1
    ...
    and vtc.cal_date between '2016-11-01' and '2024-07-01'
group by vtc.cal_date
"""
...
# sfai dataframe
compare_sfai_df_3 = read_sql_server_to_df(configs) \
  .option("query", sfai_query_3) \
  .load()

# COMMAND ----------

# rs dataframe
compare_rs_df_3 = read_redshift_to_df(configs) \
  .option("query", rs_query_3) \
  .load()

# COMMAND ----------

sfai_prep_1 = compare_sfai_df_3.toPandas()
rs_prep_1 = compare_rs_df_3.toPandas()

sfai_prep_3 = sfai_prep_1.reindex(['cal_date', 'variable', 'units'], axis=1)
rs_prep_3 = rs_prep_1.reindex(['cal_date', 'variable', 'units'], axis=1)

viz_df_1 = pd.concat([sfai_prep_3, rs_prep_3], sort=True)
viz_df_2 = viz_df_1.sort_values(by=['cal_date', 'variable'])
...
fig = px.line(data_frame=viz_df_2,
              ...
              line_group='variable',
              color='variable',
              title='RS - SFAI Base Fcst compare - INK')
