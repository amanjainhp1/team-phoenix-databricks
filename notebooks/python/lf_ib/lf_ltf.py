...
df_lf_ltf_design = spark.read.format('csv').options(header='true', inferSchema='true').load('{}product/norm_ships/fcst/ltf/large_format/lf_ltp_design.csv'.format(constants['S3_BASE_BUCKET'][dbutils.widgets.get("stack")]))
df_lf_ltf_pro = spark.read.format('csv').options(header='true', inferSchema='true').load('{}product/norm_ships/fcst/ltf/large_format/lf_ltp_pro.csv'.format(constants['S3_BASE_BUCKET'][dbutils.widgets.get("stack")]))
