...
WITH shm_01_iso AS
   ...
   , shm_02_geo AS
    (SELECT DISTINCT shm.platform_subset
     ...
     FROM stage.supplies_hw_mapping_lf AS shm     --lf_comment change to prod
...
query_list.append(["stage.shm_base_helper_lf", shm_base_helper_lf, "overwrite"])
...
SELECT 'ACTUALS'                AS source
     ...
     , CASE
           ...
           ELSE 'CARTRIDGE' END AS consumable_type
     , SUM(acts.base_quantity)  AS cartridge_volume
FROM stage.actuals_supplies_lf AS acts                                --lf_comment why not actuals_supplies_lf table in DBT
         JOIN stage.supplies_xref_lf AS xref                            --lf_comment supplies_xref_lf in stage
              ON xref.base_product_number = acts.base_product_number
         JOIN mdm.iso_cc_rollup_xref AS cref
              ON cref.country_alpha2 = acts.country_alpha2
                  AND cref.country_scenario = 'MARKET10'
...
WITH dbd_01_ib_load AS
    (SELECT ib.cal_date
          ...
          , ib.measure
          , ib.units
     FROM prod.ib AS ib                 --lf_comment how large format ib move to production table
   ...
   , dmd_02_ib AS
    (SELECT ib.cal_date
          ...
          , SUM(CASE WHEN UPPER(ib.measure) = 'PRINTER INSTALLS' THEN ib.units END) AS printer_installs    --lf_comment
    ...
    (SELECT us.geography
     ...
     WHERE 1 = 1
       ...
       SELECT us.geography
          ...
          , us.measure
          , us.units
     FROM stage.usage_share_lf AS us                                    --lf_comment add usage_share_lf (need to promote from stage to prod)
              JOIN mdm.hardware_xref AS hw
                   ON hw.platform_subset = us.platform_subset
     WHERE 1 = 1
       AND us.version = ('2021.01.19.1')
       AND UPPER(us.measure) IN
           ('USAGE', 'COLOR_USAGE', 'K_USAGE', 'HP_SHARE')
       AND UPPER(us.geography_grain) = 'MARKET10'
       AND NOT UPPER(hw.product_lifecycle_status) = 'E'
   ...
   , dmd_07_calcs AS
    (SELECT cal_date
     ...
     FROM dmd_06_us_ib ib
     inner join mdm.hardware_xref xref
        on xref.platform_subset = ib.platform_subset
     where xref.technology IN ('INK' , 'PWA', 'LASER')
     GROUP BY ib.cal_date
            , ib.geography
            , ib.platform_subset
            , ib.customer_engagement
     ...
     SELECT cal_date
        ...
        , customer_engagement
        , SUM(ib)                                                   AS ib
        , SUM(k_usage * (1-fraction_host)  * hp_share * ib)         AS HP_K_PAGES
        , SUM(k_usage * (1-fraction_host) * (1 - hp_share) * ib)    AS NON_HP_K_PAGES
        , SUM(color_usage * (1-fraction_host) * hp_share * ib)      AS HP_COLOR_PAGES
        , SUM(color_usage *
                (1 - hp_share) *
                (1-fraction_host) *
                ib)                                                 AS NON_HP_COLOR_PAGES
         , SUM(color_usage * fraction_host * ib)                    AS TOTAL_COLOR_PAGES 
         , SUM(k_usage * fraction_host * ib)                        AS TOTAL_K_PAGES      
     FROM dmd_06_us_ib ib
     inner join mdm.hardware_xref xref
        on xref.platform_subset = ib.platform_subset
     where xref.technology IN ('LF')
     GROUP BY ib.cal_date
        , ib.geography
        , ib.platform_subset
        , ib.customer_engagement
...
WITH pcm_02_hp_demand AS
   ...
   , pcm_05_ccs AS
    (SELECT DISTINCT v.cal_date
            ...
            END                            AS ccs
     FROM pcm_04_crg_actuals AS v
              JOIN stage.supplies_xref_lf  s             --lf_comment lf table
                   ON s.base_product_number = v.base_product_number
              JOIN mdm.hardware_xref AS hw
                   ON hw.platform_subset = v.platform_subset
              JOIN pcm_02_hp_demand AS dmd
   ...
   , pcm_09_mix_dmd_spread AS
    (SELECT m8.cal_date
          ...
          , m8.mix_step_1
          , m8.cc_mix
          , m8.cc_mix * m2.black_demand as device_spread
     FROM pcm_08_mix_step_3_acts AS m8
              JOIN pcm_02_hp_demand AS m2
                   ON m8.cal_date = m2.cal_date
                       AND m8.geography = m2.geography
   ...
   , supplies_forecast_months AS
    (SELECT DATEADD(MONTH, 1, MAX(sup.cal_date)) AS supplies_forecast_start
     FROM stage.actuals_supplies_lf AS sup     --lf_comment
     WHERE 1 = 1)
...
query_list.append(["stage.cc_mix_engine_lf", cc_mix_engine_lf, "overwrite"])
...
WITH pcm_27_pages_ccs_mix_prep AS
    (
        ...
        SELECT m19.type
             ...
             , m19.customer_engagement
             , m19.cc_mix
        FROM stage.cc_mix_engine_lf AS m19
                 LEFT OUTER JOIN stage.page_cc_mix_override AS m26
                                 ON m26.cal_date = m19.cal_date
                                     AND UPPER(m26.market10) = UPPER(m19.geography)
                                     AND UPPER(m26.platform_subset) = UPPER(m19.platform_subset)
...
query_list.append(["stage.lf_mix_complete", lf_mix_complete, "overwrite"])
...
SELECT type
     ...
     , cc_mix AS mix_rate
     , composite_key
FROM stage.lf_mix_complete
"""

query_list.append(["stage.page_cc_mix_lf", page_cc_mix, "overwrite"])
