...
ltf_version = spark.sql("""SELECT MAX(version) AS version FROM hardware_ltf WHERE record = 'hw_fcst'""").head()[0]
...
submit_remote_query(configs["redshift_dbname"], configs["redshift_port"], configs["redshift_username"], configs["redshift_password"], configs["redshift_url"], """CALL prod.addversion_sproc('allocated_ltf', 'wd3_allocated_flash_plus_ltf');""")
...
max_allocated_version = version.select("version").distinct().head()[0]
max_allocated_load_date = version.select("load_date").distinct().head()[0]

print("max_allocated_version: " + max_allocated_version)
print("max_allocated_load_date: " + str(max_allocated_load_date))
...
SELECT
	...
	, base_product_number
	, SUM(units) AS units
    , CAST("{max_allocated_load_date}" as date) AS load_date
	, source
	,"{max_allocated_version}" AS version
...
submit_remote_query(configs["redshift_dbname"], configs["redshift_port"], configs["redshift_username"], configs["redshift_password"], configs["redshift_url"], "UPDATE prod.hardware_ltf SET official = 0 WHERE record = 'hw_stf_fcst' AND official=1;")
