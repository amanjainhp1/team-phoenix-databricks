...
#truncate_testcases_table_query =f""" delete from qa.test_cases where module_name='Actuals Revenue';"""
...
VALUES 
('Phoenix - QA','','','Biz Validation testing','Actuals Revenue','Check if actual base product revenue is present but not IB ','fin_prod','fin_prod.actuals_supplies_baseprod','units','select platform_subset,country_alpha2  from fin_prod.actuals_supplies_baseprod
...
,('Phoenix - QA','','','Data Check','Actuals Revenue','Check for negative revenue in actuals supplies baseprod','fin_prod','fin_prod.actuals_supplies_baseprod','units','select platform_subset,country_alpha2  from fin_prod.actuals_supplies_baseprod
group by platform_subset,country_alpha2 having sum(net_revenue)<0','','1','1',getdate(),'admin',1,'Medium')
,('Phoenix - QA','','','Data Check','Actuals Revenue','Check for negative revenue in actuals supplies salesprod','fin_prod','fin_prod.actuals_supplies_salesprod','units','select country_alpha2,sales_product_number  from fin_prod.actuals_supplies_salesprod
group by country_alpha2,sales_product_number having sum(net_revenue)<0','','1','1',getdate(),'admin',1,'Medium')
,('Phoenix - QA','','','Data Check','Actuals Revenue','Check if revenue is present without units in actuals supplies salesprod','fin_prod','fin_prod.actuals_supplies_salesprod','units','select country_alpha2,sales_product_number  from fin_prod.actuals_supplies_salesprod
group by country_alpha2,sales_product_number having sum(revenue_units)=0 and sum(net_revenue)>0','','1','1',getdate(),'admin',1,'Medium')
,('Phoenix - QA','','','Data Check','Actuals Revenue','Check if revenue is present without units in actuals supplies baseprod','fin_prod','fin_prod.actuals_supplies_baseprod','units','select platform_subset,country_alpha2,base_product_number  from fin_prod.actuals_supplies_baseprod
...
,('Phoenix - QA','','','Biz Validation testing','Actuals Revenue','Check if actual supplies revenue is available but not US','fin_prod','fin_prod.actuals_supplies_baseprod','units','select platform_subset,country_alpha2  from fin_prod.actuals_supplies_baseprod
...
,('Phoenix - QA','','','Biz Validation testing','Actuals Revenue','Check for drop in PS,Country level between US and Actuals Revenue','fin_prod','fin_prod.actuals_supplies_baseprod','units','select platform_subset,geography from prod.usage_share_country where version in (select max(version) from  prod.usage_share_country )
...
group by platform_subset,country_alpha2 having sum(net_revenue)<>0','','1','1',getdate(),'admin',1,'Medium')
,('Phoenix - QA','','','Biz Validation testing','Actuals Revenue','Check for drop in PS,Country level between IB and Actuals Revenue','fin_prod','fin_prod.actuals_supplies_baseprod','units','select platform_subset,country_alpha2 from prod.ib where version in (select max(version) from  prod.ib where official=1 )
...
group by platform_subset,country_alpha2 having sum(net_revenue)<>0','','1','1',getdate(),'admin',1,'Medium')
,('Phoenix - QA','','','Duplicate check','Actuals Revenue','Check for duplicate records','prod','prod.actuals_supplies','units',' select cal_date , country_alpha2, market10, platform_subset, customer_engagement ,version,base_product_number, count(1) count_dup  from prod.actuals_supplies as2  where version = (select max(version) from prod.actuals_supplies as2 where official = 1)
 group by cal_date , country_alpha2, market10, platform_subset, customer_engagement,version,base_product_number  having count_dup >1','','1','1',getdate(),'admin',1,'Critical')
