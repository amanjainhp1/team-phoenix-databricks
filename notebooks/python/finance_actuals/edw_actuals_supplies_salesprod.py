...
FROM supplies_mps_full_calendar mps
...
-- needed for odw data only; included here as a note only; presumably edw already includes the offset but we can't identify it
SELECT 
    cal_date,
    country_alpha2,
    pl,
...
FROM fin_stage.planet_actuals AS p
...
FROM fin_stage.supplies_finance_hier_restatements_2020_2021
...
FROM fin_stage.planet_actuals planet
