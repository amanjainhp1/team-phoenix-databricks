...
WHERE pl IN 
    ...
    AND cal_date = (SELECT MAX(cal_date) FROM fin_prod.odw_sacp_actuals)
    AND cal_date > '2021-10-01'
...
update_plgd_currency = f"""
    SELECT
        ...
        revenue_units  * COALESCE(gd_proxy_mix, 1) AS revenue_units, 
        version,
        load_date
...
max_databricks_cal_date = salesprod_financials \
    .select("cal_date") \
    .distinct() \
    .rdd.flatMap(lambda x: x).collect()[0] \
    .strftime("%Y-%m-%d")
...
if max_databricks_cal_date > max_redshift_cal_date:
    ...
    raise Exception("fin_prod.odw_actuals_supplies_salesprod already contains data for cal_date: " + max_databricks_cal_date)
