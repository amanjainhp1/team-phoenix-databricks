...
#odw_revenue_units_sales_landing_media.printSchema()
...
for table in tables:
    ...
    if dbutils.widgets.get("refresh_db_data").lower() == 'true':
        df = table[1]
...
SELECT 
	(SELECT distinct record FROM odw_actuals_supplies_salesprod WHERE version = (SELECT max(version) FROM odw_actuals_supplies_salesprod)) AS record,
	cal_date,
	country_alpha2,
	market10,
	sales_product_number,
	...
	SUM(gross_profit) AS gross_profit,
	SUM(revenue_units) AS revenue_units,
	(SELECT distinct official FROM odw_actuals_supplies_salesprod WHERE version = (SELECT max(version) FROM odw_actuals_supplies_salesprod)) AS official,
	(SELECT distinct version FROM odw_actuals_supplies_salesprod WHERE version = (SELECT max(version) FROM odw_actuals_supplies_salesprod)) AS version
...
write_df_to_redshift(configs, baseprod_actuals_yields, "fin_stage.odw_actuals_supplies_baseprod_staging_interim_supplies_only", "append", postactions = "", preactions = "truncate fin_stage.odw_actuals_supplies_baseprod_staging_interim_supplies_only")
