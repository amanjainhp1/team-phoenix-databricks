...
WITH geography_mapping AS
    ...
    , override_filters AS
     (SELECT DISTINCT 'SCENARIO_USAGE_SHARE'                 AS record
      ...
      WHERE 1 = 1
        ...
        AND UPPER(us_scen.user_name) IN ('ANAA','ANA.ANAYA@HP.COM', 'SAIMANK', 'SAIMAN.KUSIN@HP.COM', 'SONSEEAHRAY', 'SONSEEAHRAY.RUCKER@HP.COM', 'ZACP', 'ZACHARY.PEAKE@HP.COM'))    --lf_comment name change
 ...
 select * from lf_us_subset
"""

query_list.append(["scen.lf_01_us_uploads", lf_01_us_uploads, "overwrite"])
...
WITH dmd_01_ib_load AS
   ...
   , dmd_03_us_load AS
    (SELECT us.geography
          ...
          , us.measure
          , us.units
     FROM scen.lf_01_us_uploads AS us
     JOIN mdm.hardware_xref AS hw
...
query_list.append(["scen.lf_02_us_dmd", lf_02_us_dmd, "overwrite"])
...
query_list.append(["scen.lf_demand", lf_demand, "overwrite"])
   ...
   , lf_usage_share AS
    (SELECT uss.geography_grain
     ...
     SELECT 'MARKET10' AS geography_grain
          ...
          , fv.us_version
          , fv.ib_version
     FROM scen.lf_demand AS us
     CROSS JOIN override_filters AS fv
     JOIN mdm.hardware_xref AS hw
         ON us.platform_subset = hw.platform_subset
     LEFT OUTER JOIN scen.lf_02_us_dmd AS us_scen
     ...
     WHERE 1 = 1
       ...
       SELECT * FROM lf_usage_share
"""

query_list.append(["scen.lf_03_usage_share", lf_03_usage_share, "overwrite"])
...
WITH shm_01_iso AS
   ...
   , shm_02_geo AS
    (SELECT DISTINCT shm.platform_subset
     ...
     FROM stage.supplies_hw_mapping_lf AS shm   --lf_comment
     JOIN mdm.hardware_xref AS hw
     ...
     SELECT DISTINCT shm.platform_subset
         ...
         WHEN hw.technology = 'LASER'
                                                                            THEN 'TRAD'
                                                                            ELSE shm.customer_engagement END AS customer_engagement
     FROM stage.supplies_hw_mapping_lf AS shm
     JOIN mdm.hardware_xref AS hw
         ON hw.platform_subset = shm.platform_subset
...
query_list.append(["scen.lf_shm_base_helper",lf_shm_base_helper, "overwrite"])
...
FROM stage.actuals_supplies_lf AS acts
JOIN stage.supplies_xref_lf AS xref
...
query_list.append(["scen.lf_cartridge_units", lf_cartridge_units, "overwrite"])
...
WITH pcm_02_hp_demand AS
    (SELECT d.cal_date
     ...
     FROM scen.lf_03_usage_share AS d
     JOIN mdm.hardware_xref AS hw
   ...
   , pcm_04_crg_actuals AS
    (SELECT DISTINCT v.cal_date
                   ...
                   , v.consumable_type
                   , v.cartridge_volume
     FROM scen.lf_cartridge_units AS v
   ...
   , pcm_05_ccs AS
    (SELECT DISTINCT v.cal_date
     ...
     WHERE 1=1 AND hw.technology IN ('LF'))
...
query_list.append(["scen.lf_page_mix_engine", lf_page_mix_engine, "overwrite"])
...
query_list.append(["scen.lf_page_cc_mix_override",lf_page_cc_mix_override, "overwrite"])
...
WITH pcm_27_pages_ccs_mix_prep AS
    (
        ...
        SELECT m19.type
             ...
             , m19.cc_mix
        FROM scen.lf_page_mix_engine AS m19
                 LEFT OUTER JOIN scen.lf_page_cc_mix_override AS m26
        ...
        FROM scen.lf_page_cc_mix_override AS m26
        JOIN mdm.hardware_xref AS hw
...
query_list.append(["scen.lf_cc_mix_complete", lf_cc_mix_complete, "overwrite"])
...
WITH geography_mapping     AS
   ...
   , override_filters      AS
    (SELECT DISTINCT 'SCENARIO_MIX_RATE'                AS record
     ...
     FROM scen.working_forecast_mix_rate AS smr                     --lf_comment
     WHERE 1 = 1
...
lf_06_mix_rate_final = """
WITH lf_mix_rate_final AS
         (SELECT DATEADD(MONTH, smr.month_num, smr.min_sys_date)    AS cal_date
          ...
          FROM scen.lf_05_mix_uploads AS smr
          JOIN stage.supplies_xref_lf AS s
          ...
          SELECT mix.cal_date
              ...
              WHEN s.single_multi = 'TRI-PACK' THEN 'MULTI'
                                               ELSE 'SINGLE' END AS single_multi
          FROM scen.lf_04_cc_mix AS mix
          JOIN stage.supplies_xref_lf AS s
              ON s.base_product_number = mix.base_product_number
          LEFT OUTER JOIN scen.lf_05_mix_uploads AS smr
...
SELECT *
FROM lf_mix_rate_final
"""

query_list.append(["scen.lf_06_mix_rate_final", lf_06_mix_rate_final, "overwrite"])
...
WITH crg_months               AS
   ...
   , pcrg_01_k_acts           AS
    (SELECT pcm.type
     ...
     FROM scen.lf_06_mix_rate_final AS pcm
     JOIN stage.supplies_xref_lf AS s
...
query_list.append(["scen.lf_07_page_cc_cartridges", lf_07_page_cc_cartridges, "overwrite"])
