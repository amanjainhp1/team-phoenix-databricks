...
# Source script:
# https://github.azc.ext.hp.com/supplies-bd/usage-share/blob/country-r-code-dev/UPM_Ink_Ctry_Update.Rmd
# https://github.azc.ext.hp.com/supplies-bd/usage-share/commit/0bc218d1263301a118f1dccce79674eb3335966c

# COMMAND ----------

# MAGIC %md
# MAGIC Version 2021.11.08.1  
# MAGIC title: "UPM Color ink at country level"  
# MAGIC output: html_notebook  
...
ib <- read.jdbc(
  url="jdbc:sqlserver://sfai.corp.hpicloud.net:1433;database=IE2_Prod",
  tableName="dbo.ib",
...
hardware_xref <- read.jdbc(
  url="jdbc:sqlserver://sfai.corp.hpicloud.net:1433;database=IE2_Prod",
  tableName="dbo.hardware_xref",
  user="databricks_user",
  password="databricksdemo123",
  driver="com.microsoft.sqlserver.jdbc.SQLServerDriver")

iso_country_code_xref <- read.jdbc(
  url="jdbc:sqlserver://sfai.corp.hpicloud.net:1433;database=IE2_Prod",
  tableName="dbo.iso_country_code_xref",
  user="databricks_user",
  password="databricksdemo123",
  driver="com.microsoft.sqlserver.jdbc.SQLServerDriver")

iso_cc_rollup_xref <- read.jdbc(
  url="jdbc:sqlserver://sfai.corp.hpicloud.net:1433;database=IE2_Prod",
  tableName="dbo.iso_cc_rollup_xref",
  user="databricks_user",
  password="databricksdemo123",
  driver="com.microsoft.sqlserver.jdbc.SQLServerDriver")
...
# read in input data
zero <- read.df("dbfs:/mnt/usage-share/ink/input/zero_c.csv", source = "com.databricks.spark.csv", header="true", inferSchema = "true")

# create views of each SparkR Dataframe
createOrReplaceTempView(zero, "zero")

# COMMAND ----------

# # comment back in when we can connect to bdbt db
# zero <- SparkR::sql(paste("
#           --Share and Usage Splits (Trad)
# WITH sub_a AS (
# 	SELECT printer_id
# 	-- Ref
# 		, CASE
# 			WHEN CHARINDEX(' ',use.platform_subset_reporting_name)=0 THEN use.platform_subset_reporting_name
# 			ELSE LEFT(use.platform_subset_reporting_name,CHARINDEX(' ',use.platform_subset_reporting_name)-1)
# 			END AS printer_short_name
# 		, use.platform_subset_reporting_name AS printer_platform_name
# 		, ref.rdma_platform_subset_name
# 		, use.hp_strategic_region_code AS printer_region_code
# 		, use.derived_country_name
# 		, use.hp_country_common_name
# 		, SUBSTRING(use.fiscal_year_quarter,1,4) AS year
# 		, SUBSTRING(use.fiscal_year_quarter,5,6) AS quarter
# 		, use.date_month_dim_ky
# 		, CASE WHEN use.printer_instant_ink_state = 'ENROLLED' THEN 'I-INK'
# 		       WHEN use.printer_instant_ink_state = 'SUBSCRIBED' THEN 'I-INK'
# 		       ELSE 'TRAD'
# 		       END
# 		       AS printer_managed
# 		, use.product_price_range
# 		, use.product_technology_type
# 		, use.product_technology_split
# 		, ref.platform_name
# 		, TRIM(regexp_replace(ref.product_model_name,'[0-9]+')) AS product_model_name
# 		, ref.product_line_code
# 		, ref.product_function_code
# 		, ref.product_business_model
# 	-- Black CCs
# 		, COALESCE(use.k_total_cc,0) AS k_total
# 		, COALESCE(use.k_no_host_cc,0) AS k_no_host
# 		, COALESCE(use.k_host_cc,0) AS k_host		
# 		, COALESCE(use.k_hp_cc,0) AS k_hp
# 		, COALESCE(use.k_refill_cc,0) AS k_refill
# 		, COALESCE(use.k_altered_cc,0) AS k_reman
# 		, COALESCE(use.k_emulated_cc,0) AS k_clone
# 		, k_refill+k_reman+k_clone AS k_nohp
# 	-- Cyan CCs
# 		, COALESCE(use.c_total_cc,0) AS c_total
# 		, COALESCE(use.c_no_host_cc,0) AS c_no_host
# 		, COALESCE(use.c_host_cc,0) AS c_host
# 		, COALESCE(use.c_hp_cc,0) AS c_hp
# 		, COALESCE(use.c_refill_cc,0) AS c_refill
# 		, COALESCE(use.c_altered_cc,0) AS c_reman
# 		, COALESCE(use.c_emulated_cc,0) AS c_clone		
# 		, c_refill+c_reman+c_clone AS c_nohp
# 	-- Magenta CCs		
# 		, COALESCE(use.m_total_cc,0) AS m_total
# 		, COALESCE(use.m_no_host_cc,0) AS m_no_host
# 		, COALESCE(use.m_host_cc,0) AS m_host
# 		, COALESCE(use.m_hp_cc,0) AS m_hp
# 		, COALESCE(use.m_refill_cc,0) AS m_refill
# 		, COALESCE(use.m_altered_cc,0) AS m_reman
# 		, COALESCE(use.m_emulated_cc,0) AS m_clone
# 		, m_refill+m_reman+m_clone AS m_nohp
# 	-- Yellow CCs
# 		, COALESCE(use.y_total_cc,0) AS y_total
# 		, COALESCE(use.y_no_host_cc,0) AS y_no_host
# 		, COALESCE(use.y_host_cc,0) AS y_host
# 		, COALESCE(use.y_hp_cc,0) AS y_hp
# 		, COALESCE(use.y_refill_cc,0) AS y_refill
# 		, COALESCE(use.y_altered_cc,0) AS y_reman
# 		, COALESCE(use.y_emulated_cc,0) AS y_clone
# 		, y_refill+y_reman+y_clone AS y_nohp
# 	-- Photo Black CCs
# 		, COALESCE(use.lk_total_cc,0) AS lk_total
# 		, COALESCE(use.lk_no_host_cc,0) AS lk_no_host
# 		, COALESCE(use.lk_host_cc,0) AS lk_host
# 		, COALESCE(use.lk_hp_cc,0) AS lk_hp
# 		, COALESCE(use.lk_refill_cc,0) AS lk_refill
# 		, COALESCE(use.lk_altered_cc,0) AS lk_reman
# 		, COALESCE(use.lk_emulated_cc,0) AS lk_clone
# 		, lk_refill+lk_reman+lk_clone AS lk_nohp		
# 	-- Color CCs
# 		, c_total+m_total+y_total+lk_total AS col_total
# 		, c_no_host+m_no_host+y_no_host+lk_no_host AS col_no_host
# 		, c_host+m_host+y_host+lk_host AS col_host
# 		, c_hp+m_hp+y_hp+lk_hp AS col_hp
# 		, c_refill+m_refill+y_refill+lk_refill AS col_refill
# 		, c_reman+m_reman+y_reman+lk_reman AS col_reman
# 		, c_clone+m_clone+y_clone+lk_clone AS col_clone
# 		, c_nohp+m_nohp+y_nohp+lk_nohp AS col_nohp		
# 	-- Total CCs
# 		, k_total+col_total AS tot_cc
# 		, COALESCE(use.total_cc,0) AS total_cc_check
# 	-- Total No Host CCs
# 		, k_no_host+col_no_host AS tot_no_host_cc
# 		, COALESCE(use.total_no_host_cc,0) AS total_no_host_cc_check		
# 	-- HP CCs
# 		, k_hp+c_hp+m_hp+y_hp+lk_hp AS tot_hp
# 		, COALESCE(use.hp_cc,0) AS hp_cc_check		
# 	-- HP Inserted CCs
# 		, COALESCE(use.fw_hp_ink_inserted_cc,0) AS inserted_hp_cc		
# 	-- Total Pages
# 		, COALESCE(use.pages_total,0) AS total_pages		
# 	-- Other
# 		, use.days
# 		, use.ib_weight*use.ratio_month1_filter AS ib_weight_m1_adj	
# 		, use.ib_weight
# 	FROM biz_trans.idst_print_share_usage use
# 	LEFT OUTER JOIN ref_enrich.product_ref ref
# 		ON (use.product_number=ref.product_number)
# 	WHERE 1=1
# 		AND use.month_order > 1
# 		AND use.printer_months >= 100 AND use.ib_weight*use.ratio_month1_filter <= 1000
# 		AND loyalty_subclass IN ('Host','HP Loyal','Disloyal','Defector','Non-HP Only','Suspect')
# 		AND use.product_technology_split NOT IN ('TIJ_2.XG1','TIJ_2.XG2 BCP','TIJ_2.XG2 MATURE','TIJ_2.XG3 GEO OJ','TIJ_2.XG3 GEO OJ MMOTR','TIJ_2.XG3 GEO STD','TIJ_2.XG3 GEO STD MMOTR','TIJ_4.0 GEKKO','TIJ_4.0 MPENNY','TIJ_4.0 OASIS GHID')
#     AND use.product_number NOT IN ('Y0G49B','Z3M46B') -- Cancelled SKU's intended for EU (CO does not want them in our data)
# 		AND use.share_printer_filter = 'New'
# 		  --Remove if on Restricted Country List (Cuba, Iran, Sudan, Syria, North Korea) or no Market Ten mapping (Antarctica, Netherlands Antilles) 
#     AND use.hp_country_common_name NOT IN ('CUBA','IRAN, ISLAMIC REPUBLIC OF','SUDAN','SYRIAN ARAB REPUBLIC','ANTARCTICA','NETHERLANDS ANTILLES')
...
# 		, printer_managed
# 		--, product_price_range
# 		--, product_technology_type
# 		--, product_technology_split
# 		--, platform_name
# 		--, product_model_name
# 		--, product_line_code
...
# 	-- Black CCs
# 		, ROUND(SUM(k_total*ib_weight_m1_adj),2) AS k_total_cc_sum	
# 		, ROUND(SUM(k_no_host*ib_weight_m1_adj),2) AS k_no_host_cc_sum
# 		, ROUND(SUM(k_host*ib_weight_m1_adj),2) AS k_host_cc_sum
# 		, ROUND(SUM(k_hp*ib_weight_m1_adj),2) AS k_hp_cc_sum
# 		, ROUND(SUM(k_refill*ib_weight_m1_adj),2) AS k_refill_cc_sum
# 		, ROUND(SUM(k_reman*ib_weight_m1_adj),2) AS k_reman_cc_sum
# 		, ROUND(SUM(k_clone*ib_weight_m1_adj),2) AS k_clone_cc_sum	
# 		, ROUND(SUM(k_nohp*ib_weight_m1_adj),2) AS k_nohp_cc_sum		
# 	-- Cyan CCs
# 		, ROUND(SUM(c_total*ib_weight_m1_adj),2) AS c_total_cc_sum	
# 		, ROUND(SUM(c_no_host*ib_weight_m1_adj),2) AS c_no_host_cc_sum
# 		, ROUND(SUM(c_host*ib_weight_m1_adj),2) AS c_host_cc_sum
# 		, ROUND(SUM(c_hp*ib_weight_m1_adj),2) AS c_hp_cc_sum
# 		, ROUND(SUM(c_refill*ib_weight_m1_adj),2) AS c_refill_cc_sum
# 		, ROUND(SUM(c_reman*ib_weight_m1_adj),2) AS c_reman_cc_sum
# 		, ROUND(SUM(c_clone*ib_weight_m1_adj),2) AS c_clone_cc_sum
# 		, ROUND(SUM(c_nohp*ib_weight_m1_adj),2) AS c_nohp_cc_sum	
# 	-- Magenta CCs		
# 		, ROUND(SUM(m_total*ib_weight_m1_adj),2) AS m_total_cc_sum		
# 		, ROUND(SUM(m_no_host*ib_weight_m1_adj),2) AS m_no_host_cc_sum
# 		, ROUND(SUM(m_host*ib_weight_m1_adj),2) AS m_host_cc_sum
# 		, ROUND(SUM(m_hp*ib_weight_m1_adj),2) AS m_hp_cc_sum
# 		, ROUND(SUM(m_refill*ib_weight_m1_adj),2) AS m_refill_cc_sum
# 		, ROUND(SUM(m_reman*ib_weight_m1_adj),2) AS m_reman_cc_sum
# 		, ROUND(SUM(m_clone*ib_weight_m1_adj),2) AS m_clone_cc_sum
# 		, ROUND(SUM(m_nohp*ib_weight_m1_adj),2) AS m_nohp_cc_sum
# 	-- Yellow CCs
# 		, ROUND(SUM(y_total*ib_weight_m1_adj),2) AS y_total_cc_sum
# 		, ROUND(SUM(y_no_host*ib_weight_m1_adj),2) AS y_no_host_cc_sum
# 		, ROUND(SUM(y_host*ib_weight_m1_adj),2) AS y_host_cc_sum
# 		, ROUND(SUM(y_hp*ib_weight_m1_adj),2) AS y_hp_cc_sum
# 		, ROUND(SUM(y_refill*ib_weight_m1_adj),2) AS y_refill_cc_sum
# 		, ROUND(SUM(y_reman*ib_weight_m1_adj),2) AS y_reman_cc_sum
# 		, ROUND(SUM(y_clone*ib_weight_m1_adj),2) AS y_clone_cc_sum
# 		, ROUND(SUM(y_nohp*ib_weight_m1_adj),2) AS y_nohp_cc_sum		
# 	-- Photo Black CCs
# 		, ROUND(SUM(lk_total*ib_weight_m1_adj),2) AS lk_total_cc_sum
# 		, ROUND(SUM(lk_no_host*ib_weight_m1_adj),2) AS lk_no_host_cc_sum
# 		, ROUND(SUM(lk_host*ib_weight_m1_adj),2) AS lk_host_cc_sum
# 		, ROUND(SUM(lk_hp*ib_weight_m1_adj),2) AS lk_hp_cc_sum
# 		, ROUND(SUM(lk_refill*ib_weight_m1_adj),2) AS lk_refill_cc_sum
# 		, ROUND(SUM(lk_reman*ib_weight_m1_adj),2) AS lk_reman_cc_sum
# 		, ROUND(SUM(lk_clone*ib_weight_m1_adj),2) AS lk_clone_cc_sum
# 		, ROUND(SUM(lk_nohp*ib_weight_m1_adj),2) AS lk_nohp_cc_sum		
# 	-- Color CCs
# 		, ROUND(SUM(col_total*ib_weight_m1_adj),2) AS col_total_cc_sum	
# 		, ROUND(SUM(col_no_host*ib_weight_m1_adj),2) AS col_no_host_cc_sum	
# 		, ROUND(SUM(col_host*ib_weight_m1_adj),2) AS col_host_cc_sum	
# 		, ROUND(SUM(col_hp*ib_weight_m1_adj),2) AS col_hp_cc_sum	
# 		, ROUND(SUM(col_refill*ib_weight_m1_adj),2) AS col_refill_cc_sum	
# 		, ROUND(SUM(col_reman*ib_weight_m1_adj),2) AS col_reman_cc_sum	
# 		, ROUND(SUM(col_clone*ib_weight_m1_adj),2) AS col_clone_cc_sum	
# 		, ROUND(SUM(col_nohp*ib_weight_m1_adj),2) AS col_nohp_cc_sum		
# 	-- Total CCs
# 		, ROUND(SUM(tot_cc*ib_weight_m1_adj),2) AS tot_cc_sum
# 		, ROUND(SUM(total_cc_check*ib_weight_m1_adj),2) AS total_cc_check_sum
# 	-- Total No Host CCs
# 		, ROUND(SUM(tot_no_host_cc*ib_weight_m1_adj),2) AS tot_no_host_cc_sum
# 		, ROUND(SUM(total_no_host_cc_check*ib_weight_m1_adj),2) AS total_no_host_cc_check_sum		
# 	-- HP CCs
# 		, ROUND(SUM(tot_hp*ib_weight_m1_adj),2) AS tot_hp_cc_sum
# 		, ROUND(SUM(hp_cc_check*ib_weight_m1_adj),2) AS hp_cc_check_sum	
# 	-- HP Inserted CCs
# 		, ROUND(SUM(inserted_hp_cc*ib_weight_m1_adj),2) AS inserted_hp_cc_sum		
# 	-- Total Pages
# 		, ROUND(SUM(total_pages*ib_weight_m1_adj),2) AS total_pages_sum		
# 	-- Other
# 		, SUM(days*ib_weight_m1_adj) AS days_ib_ext_sum
# 		, COUNT(DISTINCT printer_id) AS printer_count
# 		, sum(ib_weight) as ib_weight
# 	FROM sub_a  
# 	WHERE 1=1
# 		--AND printer_managed = 'TRADITIONAL'
# 	GROUP BY
# 		--printer_short_name
# 		printer_platform_name
# 		, rdma_platform_subset_name
...
# 	, rdma_platform_subset_name
# 	, printer_region_code
# 	, derived_country_name as country_alpha2
# 	, hp_country_common_name
# 	, year
# 	, quarter
# 	, date_month_dim_ky
...
# 	--, product_function_code
# 	--, product_business_model
# 	, tot_hp_cc_sum/nullif(tot_no_host_cc_sum,0) AS hp_share
# 	, k_total_cc_sum/nullif((days_ib_ext_sum/30.4),0) AS black_cc_ib_wtd_avg
# 	, col_total_cc_sum/nullif((days_ib_ext_sum/30.4),0) AS color_cc_ib_wtd_avg
# 	, c_total_cc_sum/nullif((days_ib_ext_sum/30.4),0) AS cyan_cc_ib_wtd_avg
# 	, m_total_cc_sum/nullif((days_ib_ext_sum/30.4),0) AS magenta_cc_ib_wtd_avg
# 	, y_total_cc_sum/nullif((days_ib_ext_sum/30.4),0) AS yellow_cc_ib_wtd_avg
# 	, tot_cc_sum/nullif((days_ib_ext_sum/30.4),0) as tot_cc_ib_wtd_avg
# 	, total_pages_sum/nullif((days_ib_ext_sum/30.4),0) AS total_pages_ib_wtd_avg
# 	, printer_count AS reporting_printers
# FROM sub_b
# WHERE 1=1
# 	--AND printer_platform_name = 'CESAR'
# 	--AND printer_region_code = 'EU'
# 	--AND date_month_dim_ky = '201905'
# 	--AND printer_managed != 'ENROLLED'
# ORDER BY date_month_dim_ky
# 	, printer_region_code
# 	, printer_platform_name
# ;
...
zero$rdma_platform_subset_name <- regexp_replace(zero$rdma_platform_subset_name, " EM | DM ", " ")

# COMMAND ----------

ib_version <- dbutils.widgets.get("IB_VERSION") #SELECT SPECIFIC VERSION

ibtable <- SparkR::sql(paste0("
...
createOrReplaceTempView(ibtable, "ibtable")
cache(ibtable)
cacheTable("ibtable")
...
ibintrodt <- SparkR::sql("
            SELECT  a.platform_subset
                    ...
                    LEFT JOIN hardware_xref d
                      ON (a.platform_subset=d.platform_subset)
                    WHERE a.measure='ib'
                      AND a.version = (select max(version) from ib)
                      AND d.technology in ('INK','PWA')
                    GROUP BY a.platform_subset
                   ")

ibintrodt$intro_yyyymm <- concat(substr(ibintrodt$intro_date,1,4),substr(ibintrodt$intro_date,6,7))

createOrReplaceTempView(ibintrodt, "ibintrodt")
cacheTable("ibintrodt")

#Get Market10 Information
country_info <- SparkR::sql("
                      WITH mkt10 AS (
...
createOrReplaceTempView(country_info, "country_info")
cacheTable("country_info")
...
## comment back in if actually needed
# zero_platform <- SparkR::sql('select distinct printer_platform_name, printer_region_code from zero order by printer_platform_name, printer_region_code')
# zero_platform$source1 <-"tri_printer_usage_sn"
# zero_platformList <- reshape2::dcast(SparkR::collect(zero_platform), printer_platform_name ~printer_region_code, value.var="source1")
...
createOrReplaceTempView(one, "one")
...
two <- SparkR::sql('select aa1.*, aa2.NormMPV 
               from 
               zero aa1 
               left outer join 
               one aa2
...
createOrReplaceTempView(two, "two")
...
three <- SparkR::sql('select platform_division_code, product_brand, market10, developed_emerging, FYearMo, rtm
...
createOrReplaceTempView(three, "three")
...
four <- SparkR::sql(paste0("select platform_division_code, product_brand, market10, developed_emerging, FYearMo, rtm 
                      , SumMPVPN
                      , SumN
                      , SumMPVPN/SumN as MUT
                      , CONCAT(platform_division_code, '_',  product_brand, '_', market10, '_', developed_emerging ,'_', rtm) as strata
                      from three
                      where SumN >=", minSize, "
                      and platform_division_code IS NOT NULL
                      order by platform_division_code, product_brand, market10, developed_emerging, FYearMo, rtm"))

createOrReplaceTempView(four, "four")
...
four_baseR <- SparkR::collect(four)

data <- list()

for (cat in unique(four_baseR$strata))
  {
    
    # every group[using strata var] in the list create a model
    d <- subset(four_baseR, strata==cat)
...
outcome0 <- createDataFrame(outcome0)

createOrReplaceTempView(outcome0, "outcome0") 

stratpl <- SparkR::sql("SELECT distinct platform_division_code from outcome0")
stratcs <- SparkR::sql("SELECT distinct product_brand from outcome0")
stratrm <- SparkR::sql("SELECT distinct rtm from outcome0")
stratmk <- SparkR::sql("SELECT distinct market10, developed_emerging from outcome0")
stratmn <- SparkR::sql("SELECT distinct month from outcome0")

createOrReplaceTempView(stratpl, "stratpl")
createOrReplaceTempView(stratcs, "stratcs")
createOrReplaceTempView(stratrm, "stratrm")
createOrReplaceTempView(stratmk, "stratmk")
createOrReplaceTempView(stratmn, "stratmn")

stratjn <- SparkR::sql("SELECT a.platform_division_code, b.product_brand, c.rtm, d.market10, d.developed_emerging, e.month  from stratpl a , stratcs b , stratrm c, stratmk d, stratmn e")

createOrReplaceTempView(stratjn, "stratjn")

stratjn <- SparkR::sql("SELECT a.platform_division_code, a.product_brand, a.rtm, a.market10, a.developed_emerging, a.month, b.region_5
                 from stratjn a
...
outcome0 <- SparkR::sql("SELECT
                      a.platform_division_code
                    , a.product_brand
                    , a.market10       
                    , a.developed_emerging
                    , a.FYearMo
                    , a.rtm              
                    , a.SumMPVPN
                    , a.SumN
...
outcome_o <- SparkR::sql("SELECT platform_division_code, product_brand, developed_emerging, month, market10, region_5, rtm, avg(b1) as b1, avg(seasonality) as seasonality 
                     from outcome0
                     group by platform_division_code, product_brand, developed_emerging, month, market10, region_5, rtm")

createOrReplaceTempView(outcome_o, "outcome_o")

outcome_r <- SparkR::sql("SELECT platform_division_code, product_brand, developed_emerging, month, region_5, rtm, avg(b1) as b1, avg(seasonality) as seasonality 
                     from outcome_o
                     group by platform_division_code, product_brand, developed_emerging, month, region_5, rtm")

outcome_a <- SparkR::sql("SELECT platform_division_code, product_brand, developed_emerging, month, rtm, avg(b1) as b1, avg(seasonality) as seasonality 
                     from outcome_o
                     group by platform_division_code, product_brand, developed_emerging, month, rtm")

outcome_b <- SparkR::sql("SELECT platform_division_code, market10, developed_emerging, month, rtm, avg(b1) as b1, avg(seasonality) as seasonality
                     from outcome_o
                     group by platform_division_code, market10, developed_emerging, month, rtm")

outcome_c <- SparkR::sql("SELECT product_brand, market10, developed_emerging, month, rtm, avg(b1) as b1, avg(seasonality) as seasonality
                     from outcome_o
                     group by product_brand, market10, developed_emerging, month, rtm")

outcome_d <- SparkR::sql("SELECT product_brand, market10, developed_emerging, month, rtm, avg(b1) as b1, avg(seasonality) as seasonality 
                     from outcome_o
                     group by product_brand, market10, developed_emerging, month, rtm")

outcome_e <- SparkR::sql("SELECT platform_division_code, product_brand,  market10, developed_emerging, month, rtm, avg(b1) as b1, avg(seasonality) as seasonality
                     from outcome_o
                     group by  platform_division_code, product_brand,  market10, developed_emerging, month, rtm")

outcome_f <- SparkR::sql("SELECT product_brand,  developed_emerging, month, rtm, avg(b1) as b1, avg(seasonality) as seasonality
                     from outcome_o
                     group by product_brand, developed_emerging, month, rtm")

outcome_w <- SparkR::sql("SELECT product_brand, month, rtm, avg(b1) as b1, avg(seasonality) as seasonality
                     from outcome_o
                     group by product_brand, month, rtm")

createOrReplaceTempView(outcome_o, "outcome_o")
createOrReplaceTempView(outcome_r, "outcome_r")
createOrReplaceTempView(outcome_a, "outcome_a")
createOrReplaceTempView(outcome_b, "outcome_b")
createOrReplaceTempView(outcome_c, "outcome_c")
createOrReplaceTempView(outcome_d, "outcome_d")
createOrReplaceTempView(outcome_e, "outcome_e")
createOrReplaceTempView(outcome_w, "outcome_w")
createOrReplaceTempView(outcome_f, "outcome_f")

outcome <- SparkR::sql(" SELECT distinct s.platform_division_code, s.product_brand, s.market10, s.developed_emerging, s.rtm, s.month
                , CASE WHEN o.b1 != 'NA' then o.b1
...
outcome$strata <- concat_ws(outcome$platform_division_code, outcome$product_brand, outcome$market10, outcome$developed_emerging, outcome$rtm, sep="_")

createOrReplaceTempView(outcome, "outcome")
...
mkt10lst <- SparkR::sql("SELECT distinct market10,SUBSTR(developed_emerging,1,1) as developed_emerging from decay where market10 is not null")
pltbrlst <- SparkR::sql("SELECT distinct platform_division_code,product_brand,rtm from decay")

createOrReplaceTempView(mkt10lst, "mkt10lst")
createOrReplaceTempView(pltbrlst, "pltbrlst")

lstjn <- SparkR::sql("SELECT
                        a.platform_division_code
                      , a.product_brand
                      , a.rtm
                      , b.market10
                      ...
                      FROM pltbrlst a
                      LEFT JOIN mkt10lst b")

createOrReplaceTempView(lstjn, "lstjn")

decay2 <- SparkR::sql('select a.platform_division_code, a.product_brand, a.market10, a.developed_emerging, a.rtm, b.strata, b.b1
                  from lstjn a
                  left join decay b
...
createOrReplaceTempView(decay2, "decay2")

decay2 <- SparkR::sql("select *, CONCAT(platform_division_code, '_' ,product_brand, '_' ,market10, '_' ,developed_emerging,'_', rtm) as strata from decay2")

decay2$b1 <- ifelse(isNull(decay2$b1),minslp,decay2$b1)
...
usage <- SparkR::sql("SELECT
    CASE WHEN rdma_platform_subset_name is not null and rdma_platform_subset_name <> '' then rdma_platform_subset_name
...
createOrReplaceTempView(usage, "usage")

# usage_platform <- SparkR::sql('select distinct printer_platform_name, printer_region_code, rtm from usage order by printer_platform_name, printer_region_code')
# usage_platform$source2 <- "tri_printer_usage_sn"

# createOrReplaceTempView(usage_platform,"usage_platform")

# usage_platformList <- reshape2::dcast(SparkR::collect(usage_platform), printer_platform_name~printer_region_code+rtm, value.var="source2")
...
iblist <- SparkR::sql("select distinct ib.platform_subset as printer_platform_name, ib.rtm, ci.country_alpha2, ci.region_5 as printer_region_code, ci.market10, substr(ci.developed_emerging,1,1) as developed_emerging, platform_division_code
                from ibtable ib left join country_info ci
                on ib.country=ci.country_alpha2")

createOrReplaceTempView(iblist,"iblist")

u2 <- SparkR::sql('with prc as (select printer_platform_name, printer_region_code,platform_division_code, developed_emerging, rtm, FYearMo
            , sum(Na) as SNA
            from usage
...
createOrReplaceTempView(sourceR, "sourceR")

# COMMAND ----------

usage2 <- SparkR::sql('select
                  aa1.printer_platform_name
...
createOrReplaceTempView(usage2, "usage2")
...
createOrReplaceTempView(usage3,"usage3")
...
usage4<-SparkR::sql('select aa1.printer_platform_name, aa1.printer_region_code, aa1.country_alpha2, aa1.market10, aa1.developed_emerging, aa1.rtm, aa1.product_brand, aa1.fyearmo, aa1.MPV, aa1.N, aa2.b1, aa1.platform_division_code
              from 
              usage3 aa1 
              inner join
              decay aa2
...
createOrReplaceTempView(usage4,"usage4")

# COMMAND ----------

# Step 16 - extracting Intro year for a given Platform n region

introYear <- SparkR::sql(" SELECT a.platform_subset as printer_platform_name, a.rtm
...
createOrReplaceTempView(introYear,"introYear")

introYear2a <- SparkR::sql('select printer_platform_name, rtm, min(intro_fyearmo) as minYear
                      FROM introYear 
                      GROUP BY printer_platform_name, rtm')

introYear2b <- SparkR::sql('select printer_platform_name, rtm, printer_region_code, min(intro_fyearmo) as minYear
                      FROM introYear 
                      GROUP BY printer_platform_name, rtm, printer_region_code')

introYear2c <- SparkR::sql('select printer_platform_name, rtm, market10, min(intro_fyearmo) as minYear
                      FROM introYear 
                      GROUP BY printer_platform_name, rtm, market10')

introYear2d <- SparkR::sql('select printer_platform_name, rtm, market10, developed_emerging, min(intro_fyearmo) as minYear
                      FROM introYear 
                      GROUP BY printer_platform_name, rtm, market10, developed_emerging')

createOrReplaceTempView(introYear2a,"introYear2a")
createOrReplaceTempView(introYear2b,"introYear2b")
createOrReplaceTempView(introYear2c,"introYear2c")
createOrReplaceTempView(introYear2d,"introYear2d")

introYear3 <- SparkR::sql('select aa1.printer_platform_name, UPPER(aa1.rtm) as rtm
                      ,aa1.printer_region_code 
                      ,aa1.market10
                      ,aa1.developed_emerging
                      ,aa1.country_alpha2
...
createOrReplaceTempView(introYear3, "introYear3")

# COMMAND ----------

# Step 17 -  Creation of Extended Usage Matrix

usage5 <- SparkR::sql('select aa1.*, 
...
nodt <- filter(usage5, isNull(usage5$Intro_FYearMo))
usage5 <- filter(usage5, !isNull(usage5$Intro_FYearMo))  
usage5 <- filter(usage5, usage5$MPV>0)

usage5$RFYearMo <- (cast(substr(usage5$fyearmo, 1,4), "integer"))*1 + (cast(substr(usage5$fyearmo, 5,6), "integer")-1)/12
usage5$RFIntroYear <- (cast(substr(usage5$Intro_FYearMo, 1,4), "integer"))*1 + (cast(substr(usage5$Intro_FYearMo, 5,6), "integer")-1)/12
...
createOrReplaceTempView(usage5, "usage5")

# COMMAND ----------

# Step 18 -  Creation of Usage Summary Matrix

usageSummary <- SparkR::sql('select printer_platform_name, rtm, product_brand, country_alpha2
...
usageSummaryMkt <- SparkR::sql('select printer_platform_name, rtm, product_brand, market10
                      , sum(NMoSI) as SumNMOSI
                      , sum(N) as SumN
                      , sum(NLnMPV) as SumNLNMPV
                      from usage5
...
usageSummaryDE <- SparkR::sql('select printer_platform_name, rtm, product_brand, market10, developed_emerging
                      , sum(NMoSI) as SumNMOSI
                      , sum(N) as SumN
                      , sum(NLnMPV) as SumNLNMPV
                      from usage5
...
usageSummaryR5 <- SparkR::sql('select printer_platform_name, rtm, product_brand, printer_region_code
                      , sum(NMoSI) as SumNMOSI
                      , sum(N) as SumN
                      , sum(NLnMPV) as SumNLNMPV
                      from usage5
...
createOrReplaceTempView(usageSummary,"usageSummary")
createOrReplaceTempView(usageSummaryMkt,"usageSummaryMkt")
createOrReplaceTempView(usageSummaryDE,"usageSummaryDE")
createOrReplaceTempView(usageSummaryR5,"usageSummaryR5")

# COMMAND ----------

# Step 19 -  decay matrix for every combinations of Plat_Nm and Region_Cd

decay2 <- SparkR::sql('select distinct printer_platform_name, country_alpha2, product_brand, rtm, b1 from usage5 order by printer_platform_name, country_alpha2')
decay2Mkt <- SparkR::sql('select printer_platform_name, market10, product_brand, rtm, sum(b1*N)/sum(N) as b1 from usage5 group by printer_platform_name, market10, product_brand, rtm')
decay2DE <- SparkR::sql('select printer_platform_name, market10,developed_emerging,product_brand, rtm, sum(b1*N)/sum(N) as b1 from usage5 group by printer_platform_name, market10,developed_emerging,product_brand, rtm')
decay2R5 <- SparkR::sql('select printer_platform_name, printer_region_code, product_brand, rtm, sum(b1*N)/sum(N) as b1 from usage5 group by printer_platform_name, printer_region_code, product_brand, rtm')

createOrReplaceTempView(decay2,"decay2")
createOrReplaceTempView(decay2Mkt,"decay2Mkt")
createOrReplaceTempView(decay2DE,"decay2DE")
createOrReplaceTempView(decay2R5,"decay2R5")

# COMMAND ----------

# Step 20 -  Initial MPV matrix creation

usageSummary2 <- SparkR::sql('select aa1.*, aa2.b1 from 
                         usageSummary aa1 
                         inner join 
                         decay2 aa2
                         on
...
usageSummary2Mkt <- SparkR::sql('select aa1.*, aa2.b1 from 
                         usageSummaryMkt aa1 
                         inner join 
                         decay2Mkt aa2
                         on
...
usageSummary2DE <- SparkR::sql('select aa1.*, aa2.b1 from 
                         usageSummaryDE aa1 
                         inner join 
                         decay2DE aa2
                         on
...
usageSummary2R5 <- SparkR::sql('select aa1.*, aa2.b1 from 
                         usageSummaryR5 aa1 
                         inner join 
                         decay2R5 aa2
                         on
...
createOrReplaceTempView(usageSummary2Mkt,"usageSummary2Mkt")
createOrReplaceTempView(usageSummary2DE,"usageSummary2DE")
createOrReplaceTempView(usageSummary2R5,"usageSummary2R5")
createOrReplaceTempView(usageSummary2,"usageSummary2")

usageSummary2Drop <- usageSummary2[usageSummary2$SumN <50,]
usageSummary2MktDrop <- usageSummary2Mkt[usageSummary2Mkt$SumN <50,]
usageSummary2DEDrop <- usageSummary2DE[usageSummary2DE$SumN <50,]
usageSummary2R5Drop <- usageSummary2R5[usageSummary2R5$SumN <50,]

createOrReplaceTempView(usageSummary2Drop,"usageSummary2Drop")
createOrReplaceTempView(usageSummary2MktDrop,"usageSummary2MktDrop")
createOrReplaceTempView(usageSummary2DEDrop,"usageSummary2DEDrop")
createOrReplaceTempView(usageSummary2R5Drop,"usageSummary2R5Drop")

usageSummary2Keep <- SparkR::sql("select * from usageSummary2 where printer_platform_name not in (select distinct printer_platform_name from usageSummary2Drop)")
usageSummary2MktKeep <- SparkR::sql("select * from usageSummary2Mkt where printer_platform_name not in (select distinct printer_platform_name from usageSummary2MktDrop)")
usageSummary2DEKeep <- SparkR::sql("select * from usageSummary2DE where printer_platform_name not in (select distinct printer_platform_name from usageSummary2DEDrop)")
usageSummary2R5Keep <- SparkR::sql("select * from usageSummary2R5 where printer_platform_name not in (select distinct printer_platform_name from usageSummary2R5Drop)")
...
usageSummary2Keep <- usageSummary2[usageSummary2$SumN >=50,]

createOrReplaceTempView(usageSummary2Keep,"usageSummary2Keep")

# COMMAND ----------

# Step 21 -  De-normalizing Initial MPV matrix

# usageSummary2Tctry <- createDataFrame(reshape2::dcast(SparkR::collect(usageSummary2Keep), printer_platform_name+product_brand+rtm ~country_alpha2, value.var="iMPV",fun.aggregate = mean))
usageSummary2Tmkt10 <- createDataFrame(reshape2::dcast(SparkR::collect(usageSummary2MktKeep), printer_platform_name+product_brand+rtm ~market10, value.var="iMPV",fun.aggregate = mean))
usageSummary2Tde<-createDataFrame(reshape2::dcast(SparkR::collect(usageSummary2DEKeep),printer_platform_name+product_brand+rtm ~market10+developed_emerging,value.var="iMPV",fun.aggregate=mean))
usageSummary2Tr5 <- createDataFrame(reshape2::dcast(SparkR::collect(usageSummary2R5Keep), printer_platform_name+product_brand+rtm ~printer_region_code, value.var="iMPV",fun.aggregate = mean))

# createOrReplaceTempView(usageSummary2Tctry,"usageSummary2Tctry")
createOrReplaceTempView(usageSummary2Tmkt10,"usageSummary2Tmkt10")
createOrReplaceTempView(usageSummary2Tde,"usageSummary2Tde")
createOrReplaceTempView(usageSummary2Tr5,"usageSummary2Tr5")
...
createOrReplaceTempView(strata,"strata")

usageSummary2TEr5 <- SparkR::sql('select aa1.printer_platform_name, aa1.product_brand, aa2.platform_division_code, aa1.rtm, aa1.NA, aa1.EU, aa1.AP, aa1.LA
...
usageSummary2TEr5_baseR <- SparkR::collect(usageSummary2TEr5)
usageSummary2TEr5_baseR[usageSummary2TEr5_baseR == 0] <- NA 
usageSummary2TEr5 <- createDataFrame(usageSummary2TEr5_baseR)

createOrReplaceTempView(usageSummary2TEr5,"usageSummary2TEr5")

usageSummary2TEmkt10 <- SparkR::sql('select aa1.printer_platform_name, aa1.product_brand, aa2.platform_division_code, aa1.rtm, aa1.`Central Europe`, aa1.`Greater Asia`, aa1.`Greater China`, aa1.`India SL & BL`, aa1.`ISE`, aa1.`Latin America`, aa1.`North America`, aa1.`Northern Europe`, aa1.`Southern Europe`, aa1.`UK&I`
                           --North America
...
usageSummary2TEmkt10_baseR <- SparkR::collect(usageSummary2TEmkt10)
usageSummary2TEmkt10_baseR[usageSummary2TEmkt10_baseR == 0] <- NA 
usageSummary2TEmkt10 <- createDataFrame(usageSummary2TEmkt10_baseR)

createOrReplaceTempView(usageSummary2TEmkt10, "usageSummary2TEmkt10")

usageSummary2TEde <- SparkR::sql("select aa1.printer_platform_name, aa1.product_brand, aa2.platform_division_code, aa1.rtm,aa1.`Central Europe_D`,aa1.`Central Europe_E`
...
usageSummary2TEde_baseR <- SparkR::collect(usageSummary2TEde)
usageSummary2TEde_baseR[usageSummary2TEde_baseR == 0] <- NA 
usageSummary2TEde <- createDataFrame(usageSummary2TEde_baseR)

createOrReplaceTempView(usageSummary2TEde, "usageSummary2TEde")

usageSummary2TE <- SparkR::sql("select a.`printer_platform_name`, a.product_brand, a.platform_division_code, a.rtm
...
createOrReplaceTempView(usageSummary2TE,"usageSummary2TE")

# COMMAND ----------

wtaverage <- SparkR::sql('select platform_division_code, rtm, product_brand
                     , avg(EU_wt) as EUa
...
wtaverage <- fillna(wtaverage, 1)

createOrReplaceTempView(wtaverage,"wtaverage")

# COMMAND ----------

usageSummary2TE_D2<-SparkR::sql('select aa1.*,aa2.`EUa`,aa2.`Central_Europena`,aa2.`Central_Europe_Dna`,aa2.`Central_Europe_Ena`,aa2.`Northern_Europena`
...
createOrReplaceTempView(usageSummary2TE_D2,"usageSummary2TE_D2")

   usageSummary2TE_D3 <- SparkR::sql("select *
                                , CASE 
...
createOrReplaceTempView(usageSummary2TE_D3,"usageSummary2TE_D3")

   #Using Market10, can switch to Market10D/E?
      sourceRiMPV <- SparkR::sql("
...
createOrReplaceTempView(sourceRiMPV, "sourceRiMPV")
...
createOrReplaceTempView(usagesummaryNAEUAP, "usagesummaryNAEUAP")
...
PoRtst <- SparkR::sql(q4)
PoRtst$product_usage_por_pages <- cast(PoRtst$product_usage_por_pages, "double")  
PoRtst$print_mono_speed_page <- cast(PoRtst$print_mono_speed_pages, "double")  
PoRtst$print_color_speed_pages <- cast(PoRtst$print_color_speed_pages, "double")

createOrReplaceTempView(PoRtst, "PoRtst")

POR_porest <- summarize(groupBy(zero, zero$product_line_code), avg_usage = avg(zero$usage))

createOrReplaceTempView(POR_porest, "POR_porest")
...
PoR <- SparkR::sql("
             ...
             ,CASE WHEN a.product_usage_por_pages is not null then a.product_usage_por_pages
              ...
              END AS product_usage_por_pages
             from PoRtst a 
             left join POR_porest b on a.product_line_code=b.product_line_code
             ")

createOrReplaceTempView(PoR, "PoR")
...
PoR_platformLIST <- SparkR::sql('select distinct printer_platform_name,product_brand,"product_ref" as Source from PoR')

createOrReplaceTempView(PoR_platformLIST,"PoR_platformLIST")

# COMMAND ----------

# Step - 50 -- extracting platform specific general IntroDate and create Old-Future type

old <- SparkR::sql(paste0("
   WITH ibset as (
...
createOrReplaceTempView(old, "old")

#   # "dropList" is the list of platforms which are recorded both as Old and Future
  
#   dropList <- old$platform_type=="FUTURE",$printer_platform_name %in% (usagesummaryNAEUAP$printer_platform_name); dropList

# COMMAND ----------

# Step - 51 attaching introdate, platform type with PoR table
 
PoR2 <- SparkR::sql('select aa1.*, aa2.rtm , aa2.INTRODATE_NA, aa2.INTRODATE_NE, aa2.INTRODATE_SE, aa2.INTRODATE_CED, aa2.INTRODATE_CEE, aa2.INTRODATE_UK
...
PoR2$J90Mo <- (cast(substr(PoR2$Intro_FYearMo, 1,4), "integer") - 1990)*12 + (cast(substr(PoR2$Intro_FYearMo, 5,6), "integer")-1)
...
PoR2B <- PoR2
PoR2B$product_intro_price <- cast(PoR2B$product_intro_price, "double")

# COMMAND ----------

# Step - 52 Creating raw_iMPV for all old Platforms using model information
PoR2model <- SparkR::collect(PoR2B)
...
createOrReplaceTempView(as.DataFrame(PoR2model), "PoR2model")

# COMMAND ----------

# Step - 53 attaching calculated_iMPV with raw_iMPV data

PoR2model_iMPV <- SparkR::sql('select aa1.*, aa2.NA2, aa2.EU2, aa2.AP2, aa2.LA2, North_America2, UKI2, Northern_Europe2, Southern_Europe2, ISE2, Central_Europe2, India2, Greater_Asia2, Greater_China2,Latin_America2, North_America_D2, UKI_D2, Northern_Europe_D2, Southern_Europe_D2, ISE_E2, Central_Europe_D2, Central_Europe_E2, India_E2, Greater_Asia_D2, Greater_Asia_E2,
...
createOrReplaceTempView(PoR2model_iMPV, "PoR2model_iMPV")
...
# Step - 55 Calculating the average of iMPV ratio specific to groups defined by SM, CM
  ...
  avgRatio1 <- SparkR::sql('select platform_division_code, product_brand, rtm, avg(ratio1) as avgratio1 from PoR2model_iMPV group by platform_division_code, product_brand, rtm')
  avgRatio2 <- SparkR::sql('select platform_division_code, product_brand, rtm, avg(ratio2) as avgratio2 from PoR2model_iMPV group by platform_division_code, product_brand, rtm')
  avgRatio3 <- SparkR::sql('select platform_division_code, product_brand, rtm, avg(ratio3) as avgratio3 from PoR2model_iMPV group by platform_division_code, product_brand, rtm')

createOrReplaceTempView(avgRatio1, "avgRatio1")
createOrReplaceTempView(avgRatio2, "avgRatio2")
createOrReplaceTempView(avgRatio3, "avgRatio3")

# COMMAND ----------

# Step - 56 attaching the average iMPV ratio

PoR2model_iMPV2 <- SparkR::sql(' select aa1.*, aa2.avgratio1, aa3.avgratio2, aa4.avgratio3
                           from 
                           PoR2model_iMPV aa1
                           inner join 
                           avgRatio1 aa2
...
createOrReplaceTempView(PoR2model_iMPV2, "PoR2model_iMPV2")

PoR2model_iMPV3 <- SparkR::sql('select *
                           , case
...
createOrReplaceTempView(PoR2model_iMPV3, "PoR2model_iMPV3")

# COMMAND ----------

 # ---- Step - 58 attaching regional coefficients -------------------------------------------#

PoR2model_iMPV4 <- SparkR::sql('select aa1.*, 1 as NAa, aa2.EUa, aa2.APa, aa2.LAa, 1 as North_Americana, aa2.UKIna, aa2.Northern_Europena, aa2.Southern_Europena, aa2.ISEna, aa2.Central_Europena, aa2.Indiana, aa2.Greater_Asiana, aa2.Greater_Chinana,aa2.Latin_Americana, aa2.Greater_Asia_Dna, aa2.Greater_Asia_Ena, aa2.Greater_China_Dna, aa2.Greater_China_Ena, aa2.Central_Europe_Dna, aa2.Central_Europe_Ena
...
createOrReplaceTempView(PoR2model_iMPV4, "PoR2model_iMPV4")

PoR2model_iMPV4a <- SparkR::sql("SELECT product_brand, UPPER(rtm) as rtm, avg(NAa) as NAa, avg(EUa) as EUa, avg(APa) as APa, avg(LAa) as LAa, avg(North_Americana) as North_Americana
      ...
      GROUP BY product_brand, UPPER(rtm)
                            ")
createOrReplaceTempView(PoR2model_iMPV4a, "PoR2model_iMPV4a")

PoR2model_iMPV4 <- SparkR::sql("select aa1.*
...
PoR2model_iMPV5 <- SparkR::sql('select *
...
createOrReplaceTempView(PoR2model_iMPV5, "PoR2model_iMPV5")

# COMMAND ----------

# Step 60 - Populating the route matrix for iMPV creation based on modeled values

route5 <- SparkR::sql("select platform_division_code, product_brand,printer_platform_name,rtm
                  , case 
...
route5_baseR <- SparkR::collect(route5)
route5_baseR <- reshape2::melt(route5_baseR , id.vars = c("platform_division_code", "product_brand","printer_platform_name","rtm"),variable.name = "printer_region_code", value.name = "Route")

route5_baseR <- route5_baseR[which((route5_baseR$Route=="Modeled")|is.na(route5_baseR$Route)),]
route5_baseR$printer_platform_name <- factor(route5_baseR$printer_platform_name)
route5B <- as.DataFrame(route5_baseR)

createOrReplaceTempView(route5B, "route5B")

# COMMAND ----------

#  Step 61 - Calculating percentage of platform installed base across regions

new1 <- SparkR::sql("SELECT distinct platform_subset AS printer_platform_name 
                   , platform_division_code
                   , product_brand
                   FROM hw_info
                    ")

createOrReplaceTempView(new1, "new1")

new1b <- SparkR::sql("SELECT a.*
              , c.market10
...
new1b <- SparkR::filter(new1b, !isNull(new1b$market10))

createOrReplaceTempView(new1b, "new1b")

new1c <- SparkR::sql("SELECT platform_division_code, product_brand, printer_platform_name, rtm as rtm, market10, SUBSTR(developed_emerging,1,1) as developed_emerging
                  , sum(printer_installed_base_month) as sumINSTALLED_BASE_COUNT
               FROM
               new1b
                where printer_installed_base_month IS NOT NULL
               group by platform_division_code, product_brand, printer_platform_name, rtm, market10, developed_emerging
               order by platform_division_code, product_brand, printer_platform_name, rtm, market10, developed_emerging")

new1c$sumINSTALLED_BASE_COUNT <- cast(new1c$sumINSTALLED_BASE_COUNT, "double")

createOrReplaceTempView(new1c, "new1c")

new1cc <- SparkR::sql("SELECT platform_division_code, product_brand, printer_platform_name, rtm as rtm
                  , sum(printer_installed_base_month) as sumINSTALLED_BASE_COUNT
               FROM
               new1b
                where printer_installed_base_month IS NOT NULL
...
new1cc$sumINSTALLED_BASE_COUNT <- cast(new1cc$sumINSTALLED_BASE_COUNT, "double")

createOrReplaceTempView(new1cc, "new1cc")

new1c_baseR <- SparkR::collect(new1c)
new1d <- reshape2::dcast(new1c_baseR, platform_division_code+product_brand+printer_platform_name+rtm ~ market10+developed_emerging, value.var = "sumINSTALLED_BASE_COUNT", fun.aggregate=sum)

createOrReplaceTempView(as.DataFrame(new1d), "new1d")

new <- SparkR::sql("SELECT a.* ,b.sumINSTALLED_BASE_COUNT 
...
createOrReplaceTempView(new, "new")

# COMMAND ----------

# Step 62 - Attaching PoR with the percent installed base for OLD platforms

new2 <- SparkR::sql('select aa1.*, aa2.product_usage_por_pages, aa2.platform_type
                from new aa1
                inner join 
                PoR2model aa2
                on
...
createOrReplaceTempView(new2, "new2")
...
wtaverage2 <- SparkR::sql("select product_brand, platform_division_code, rtm, avg(EUa) as EUa, avg(`Central_Europena`) as `Central_Europena`, avg(`Central_Europe_Dna`) as `Central_Europe_Dna` 
...
createOrReplaceTempView(wtaverage2, "wtaverage2")

new3 <- SparkR::sql('select distinct aa1.*, aa2.`Central_Europena`,aa2.`Central_Europe_Dna`,aa2.`Central_Europe_Ena`,aa2.`Northern_Europena`,aa2.`Southern_Europena`,aa2.`ISEna`
...
createOrReplaceTempView(new3, "new3")
...
decay_baseR <- SparkR::collect(decay)

decayT <- reshape2::dcast(decay_baseR, platform_division_code + product_brand +rtm  ~market10+developed_emerging, value.var="b1")

createOrReplaceTempView(as.DataFrame(decayT), "decayT")

# COMMAND ----------

new4 <- SparkR::sql('select distinct aa1.*
...
route6 <- SparkR::sql('select platform_division_code, product_brand, printer_platform_name, rtm
...
route6_r <- SparkR::collect(route6)
  
route6B <- reshape2::melt(route6_r , id.vars = c("platform_division_code", "product_brand","printer_platform_name", "rtm"),variable.name = "printer_region_code", value.name = "Route")
route6B <- as.DataFrame(route6B)

usagesummaryNAEUAP_r <- SparkR::collect(usagesummaryNAEUAP)
route1 <- usagesummaryNAEUAP_r[c(2,4,3,1, 510, 513, 515, 517, 519, 521, 522, 525, 527, 528, 530, 531, 534)]
...
createOrReplaceTempView(as.DataFrame(route1B), "route1B")

route1B <- SparkR::sql("SELECT distinct platform_division_code, product_brand, printer_platform_name, rtm
...
route <- SparkR::filter(route, !isNull(route$Route))
  
route_r <- SparkR::collect(route)
routeT <- reshape2::dcast(route_r, platform_division_code + product_brand +printer_platform_name +rtm ~printer_region_code, value.var="Route")

createOrReplaceTempView(as.DataFrame(routeT), "routeT")

# COMMAND ----------

# Step - 69 - combining old and future iMPV matrix to create complete iMPV matrix

normdataOLD <- SparkR::sql('select distinct platform_division_code, product_brand, rtm 
                     , printer_platform_name
...
normdataNew <- SparkR::sql('select distinct platform_division_code, product_brand, rtm
                     , printer_platform_name
                     , NA3 AS NA
                     , NE3 AS NE
                     , SE3 AS SE
...
createOrReplaceTempView(normdata, "normdata")

# COMMAND ----------

# Step - 70 Normalize complete iMPV matrix with respect to VV, CM, SM and Plat_Nm

normdata_r <- SparkR::collect(normdata)
normdata2 <- reshape2::melt(normdata_r, id.vars = c("platform_division_code", "product_brand", "printer_platform_name", "rtm"),
                  variable.name = "printer_region_code", 
                  value.name = "iMPV")

createOrReplaceTempView(as.DataFrame(normdata2), "normdata2")

# COMMAND ----------

# Step - 71 Extracting region specific introDate for each of the platforms
 
normdatadate <- SparkR::sql('select platform_division_code, product_brand, rtm
                      , printer_platform_name
...
combined <- SparkR::sql('select aa1.PLATFORM_TYPE
...
createOrReplaceTempView(combined, "combined")

combined2 <- SparkR::sql('select aa2.*
                   , aa1.NA as NA_iMPV
                   , aa1.NE as NE_iMPV
                   , aa1.SE as SE_iMPV
                   , aa1.CED as CED_iMPV
...
normdatadate_baseR <- SparkR::collect(normdatadate)
normdatadate2 <- reshape2::melt(normdatadate_baseR, id.vars = c("platform_division_code", "product_brand", "printer_platform_name", "rtm"),
                      variable.name = "printer_region_code", 
                      value.name = "introdate")

createOrReplaceTempView(as.DataFrame(normdatadate2), "normdatadate2")

# COMMAND ----------

# Step - 74 Combine Normalized introDate and iMPV data for each Platform

sourceRiMPV$mde <- ifelse(sourceRiMPV$market10=='Central Europe',concat(lit('CE'),substr(sourceRiMPV$developed_emerging,1,1)),
                   ifelse(sourceRiMPV$market10=='Greater Asia',concat(lit('GA'),substr(sourceRiMPV$developed_emerging,1,1)), 
                   ifelse(sourceRiMPV$market10=='Greater China',concat(lit('GC'),substr(sourceRiMPV$developed_emerging,1,1)),
...
createOrReplaceTempView(normdataFinal0, "normdataFinal0")
  
normdataFinal <- SparkR::sql('select aa1.*, aa2.minYear as introdate from normdataFinal0 aa1
                       inner join
                       introYear2a aa2
                       on
                       aa1.printer_platform_name = aa2.printer_platform_name
...
normdataFinal$strata1 <- concat_ws(normdataFinal$platform_division_code, normdataFinal$product_brand, normdataFinal$market10, normdataFinal$developed_emerging, normdataFinal$rtm, sep = "_")
normdataFinal$strata2 <- concat_ws(normdataFinal$platform_division_code, normdataFinal$product_brand, normdataFinal$country_alpha2, normdataFinal$rtm, sep = "_")
...
outcome_filtered <- SparkR::filter(outcome, !isNull(outcome$strata))

normdataFinal_filtered <- SparkR::filter(normdataFinal, !isNull(normdataFinal$introdate) & !isNull(normdataFinal$iMPV) & !isNull(normdataFinal$strata1) & !isNull(normdataFinal$printer_platform_name) & !isNull(normdataFinal$rtm) & !isNull(normdataFinal$country_alpha2))

# for loop 0 replacement
d0 <- normdataFinal_filtered
d0$printerrtm <- concat_ws(sep="_", d0$aa1.printer_platform_name, d0$aa1.rtm)

d1 <- d0

d2 <- SparkR::merge(S3, d1, by = NULL)
...
write.df(final9, path=paste0("dbfs:/mnt/usage-share/output/upm_ink_ctry_", format(Sys.Date(), format="%Y%m%d"), "/"), source="csv", mode="overwrite")
